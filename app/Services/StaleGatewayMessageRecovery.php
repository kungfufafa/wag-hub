...
{
    /**
     * @return array{examined: int, requeued: int, accepted: int, outcome_unknown: int, failed: int}
     */
    public function recover(int $minutes, int $limit): array
    {
        $cutoff = now()->subMinutes($minutes);
        $messageIds = GatewayMessage::query()
            ->where('mode', 'async')
        ...
        foreach ($messageIds as $messageId) {
            $outcome = DB::transaction(
                fn (): ?string => $this->recoverOne((int) $messageId, $cutoff),
            );
    ...
    private function recoverOne(int $messageId, Carbon $cutoff): ?string
    {
        ...
        if ($attempt === null) {
            return $this->requeueWithoutAttempt($message);
        }

        if ($attempt->status === 'started') {
            return $this->reconcileStartedAttempt($message, $attempt);
        }
    ...
    private function requeueWithoutAttempt(GatewayMessage $message): string
    {
        ...
        DispatchGatewayMessage::dispatch($message->id)->afterCommit();
